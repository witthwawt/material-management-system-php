...
<body>
<?
$sql = "select * from stock_unit where su_id = '".$_GET["su_id"]."'";
$result = mysql_query($sql) or die (mysql_error());
$show = mysql_fetch_array($result);
?>

<center><h2>แก้ไขหน่วยนับ</h2>
 <form name="frm2" method="post" action="" class="form-inline" onSubmit="JavaScript:return fncSubmit();">
  <input type="text" class="form-control" name="txt1" id="txt1" value="<?=$show["su_name"];?>" style="width: 300px;ext-align: center;"><br>
...
if( isset($_POST["btn1"])){
	if($_POST['btn1'] == "Submit"){
			...
			$strSQL = "update stock_unit set su_name = '".$_POST["txt1"]."' where su_id = '".$_GET["su_id"]."'";
	...
	<script language=javascript>
		window.opener.location.reload('mail.php?mn=unit');window.close();
		</script>
		<?
		}
	}
